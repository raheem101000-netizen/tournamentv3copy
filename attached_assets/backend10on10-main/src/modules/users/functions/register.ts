...
export const register = async (args: {
  ...
  const fullName = args.user.fullName;
  ...
  const fullNameToCheck = await MongoOrb('UserCollection').collection.findOne({ fullName });
  if (fullNameToCheck) {
    return {
      hasError: RegisterErrors.FULLNAME_EXISTS,
    };
  }
  ...
  try {
    ...
    await MongoOrb('UserCollection').collection.deleteOne({ _id: insertedUser.insertedId });
    await MongoOrb('UserAuthorizationCollection').collection.deleteOne({ userId: insertedUser.insertedId });
    throw error;
  }
};
